...
import csv
graph_address = "https://graphdb.interactions.ics.unisg.ch/repositories/was-assignment-3?query="
...
def execute_SPARQL_Query(string_query): 
    encoded_query = urllib.parse.quote_plus(string_query)
    query = graph_address + encoded_query
    response = requests.get(query, auth=(user, passcode), verify=True)
    try:
...
def main():
    fileName = input("Name of the SPARQL Query file: ")
    string_query = readfile(fileName)
    sparql_query_results = execute_SPARQL_Query(string_query)
